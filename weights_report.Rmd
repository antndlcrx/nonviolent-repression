...
pacman::p_load(tidyverse, readr, broom, 
               ...
               gridExtra, knitr, haven, nnet, stargazer)
...
To compare our samples to the population, we do two comparisons. First, we compare them to a nationally representative Levada omnibus survey. Second, we compare them to the 2020 Census. 
...
shares_survey_sept <- calculate_shares(survey_sept, "Q. September")
...
```{r}
models = list(missing_t1, missing_t2, missing_t3)
stargazer(models,
          title = "Missings Check Experiments",
          digits=2,
          header=FALSE,
          no.space = TRUE)
...
```{r}
# write_csv(survey_sept, "data/surveys/survey_sept_2024_with_weights.csv")
```
