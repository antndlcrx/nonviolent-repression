...
# load survey data
survey_march <- read_csv("data/surveys/survey_march_post_election.csv", locale = locale(encoding = "UTF-8")) %>% 
...
# helper funcitons
calculate_shares <- function(dataset, variable_name) {
  dataset %>%
    group_by(!!sym(variable_name)) %>%
    summarise(count = n()) %>%
    mutate(share = count / sum(count)) %>%
    select(!!sym(variable_name), share)
}
```
