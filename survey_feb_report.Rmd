...
colnames(ru_population_frame) <- c("gender", "age_group", "university_education",
                                   "Freq")
# manually change pop values from zero to prevent models from collapsing (CONTROVERSIAL)
...
# save weights 
survey_aug$weights <- weights(weighted)
summary(weights(weighted))
...
survey_feb_selected <- survey_feb %>%
  ...
  select(weights, Survey)

survey_aug_selected <- survey_aug %>%
  mutate(Survey = 'August') %>%
  select(weights, Survey)

combined_survey <- rbind(survey_feb_selected, survey_aug_selected)

ggplot(combined_survey, aes(x = weights, fill = Survey)) +
  geom_histogram(data = subset(combined_survey, Survey == 'February'), bins = 100, alpha = 0.5, color = "black") +
  geom_histogram(data = subset(combined_survey, Survey == 'August'), bins = 100, alpha = 0.5, color = "black") +
...
```{r echo=FALSE}
plot_feb <- ggplot(survey_feb_selected, aes(x = weights)) +
...
plot_aug <- ggplot(survey_aug_selected, aes(x = weights)) +
